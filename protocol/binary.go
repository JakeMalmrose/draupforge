...
func encodeActors(w *bwriter, base, view []ActorSnap) {
	...
	for _, c := range changes {
		...
		if c.mask&actorPassives != 0 {
			w.uv(uint64(len(a.Passives)))
			for _, ps := range a.Passives {
				w.str(ps)
			}
		}
		if c.mask&actorFlasks != 0 {
			w.uv(uint64(len(a.Flasks)))
			for _, ch := range a.Flasks {
				w.sv(ch)
			}
		}
...
func actorMask(b, a *ActorSnap) uint64 {
	...
	if !reflect.DeepEqual(b.Passives, a.Passives) {
		mask |= actorPassives
	}
...
func decodeActors(r *breader, base []ActorSnap) []ActorSnap {
	...
	for n := r.uv(); n > 0 && r.err == nil; n-- {
		...
		if mask&actorPassives != 0 {
			for m := r.uv(); m > 0 && r.err == nil; m-- {
				a.Passives = append(a.Passives, r.str())
			}
		}
		if mask&actorFlasks != 0 {
			for m := r.uv(); m > 0 && r.err == nil; m-- {
...
func mergeActor(b ActorSnap, d actorDelta) ActorSnap {
	...
	if d.mask&actorPassives != 0 {
		a.Passives = n.Passives
	}
